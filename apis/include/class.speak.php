...
class speak extends DB
{
    ...
    {   
        $sql="select name,city,email,mobile,pimage,opinion,final_opinion from tbl_speak_master where active_flag=1 order by date_time DESC";
        $res =$this->query($sql);            
    ...
    {   
       ...
       if($proErr['errCode']== 0)
       { 
        $vsql="INSERT INTO tbl_speak(name,city,mobile,email,pimage,opinion,final_opinion,active_flag,date_time)
               VALUES('".$detls['name']."','".$detls['city']."',".$detls['mobile'].",'".$detls['email']."','".$detls['pimage']."','".$detls['opinion']."','".$detls['final_opinion']."',1,now())";
